...
def rnn_step_backward(dnext_h, cache):
    ...
    N,H = prev_h.shape
    dout4 = (np.ones((N,H)) - np.square(np.tanh(temp))) * dnext_h
    db = np.sum(dout4,axis=0)
    dprev_h = dout4.dot(Wh.T)
    dWh = prev_h.T.dot(dout4)
    dWx = x.T.dot(dout4)
    dx = dout4.dot(Wx.T)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    N, H = prev_h.shape
    dnextc_final = dnext_c + dnext_h * o * (1 - np.tanh(next_c)**2) 
    dprev_c = dnextc_final * f
    df = dnextc_final * prev_c  
    di = dnextc_final * g
    dg = dnextc_final * i
...
def lstm_backward(dh, cache):
    ...
    for k in range(T):
        i = T - 1 - k
        dx_tmp,dnext_h,dnext_c,dWx_tmp,dWh_tmp,db_tmp = lstm_step_backward(dnext_h + dh[:,i,:], dnext_c, cache[i])
        dx[:,i,:] += dx_tmp
        dWx += dWx_tmp
        dWh += dWh_tmp
        db += db_tmp
